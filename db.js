  ...
  try {
    ...
    await pool.request().query(`
      ...
      CREATE TABLE Product (
          id VARCHAR(50) PRIMARY KEY, name VARCHAR(255) NOT NULL, packSize VARCHAR(50), mrp DECIMAL(10, 2),
          sellingPrice DECIMAL(10, 2), discount DECIMAL(5, 2), description TEXT, imageFallback VARCHAR(50),
          stock INT, stockStatus VARCHAR(50), totalSold INT, active BIT
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='DeliveryPartner' and xtype='U')
      CREATE TABLE DeliveryPartner (
          id VARCHAR(50) PRIMARY KEY, name VARCHAR(255) NOT NULL, phone VARCHAR(20) NOT NULL,
          isActive BIT DEFAULT 1, availabilityStatus VARCHAR(50) DEFAULT 'AVAILABLE',
          createdAt DATETIME DEFAULT GETDATE(), updatedAt DATETIME DEFAULT GETDATE()
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Customer' and xtype='U')
      CREATE TABLE Customer (
          id VARCHAR(50) PRIMARY KEY, name VARCHAR(255) NOT NULL, phone VARCHAR(20) NOT NULL, pincode VARCHAR(10)
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Address' and xtype='U')
      CREATE TABLE Address (
          id VARCHAR(50) PRIMARY KEY, customerId VARCHAR(50) FOREIGN KEY REFERENCES Customer(id),
          name VARCHAR(255) NOT NULL, phone VARCHAR(20) NOT NULL, houseNumber VARCHAR(50), buildingName VARCHAR(255),
          street VARCHAR(255), area VARCHAR(255), landmark VARCHAR(255), city VARCHAR(100), state VARCHAR(100),
          pincode VARCHAR(10), addressType VARCHAR(50), latitude FLOAT, longitude FLOAT
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Coupon' and xtype='U')
      CREATE TABLE Coupon (
          id VARCHAR(50) PRIMARY KEY, code VARCHAR(50) NOT NULL, type VARCHAR(50), discountType VARCHAR(50),
          discountValue DECIMAL(10, 2), minimumOrderValue DECIMAL(10, 2), maximumDiscount DECIMAL(10, 2),
          usageLimit INT, usedCount INT DEFAULT 0, active BIT DEFAULT 1, influencerId VARCHAR(50), influencerName VARCHAR(255)
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Order' and xtype='U')
      CREATE TABLE [Order] (
          id VARCHAR(100) PRIMARY KEY, customerId VARCHAR(50) FOREIGN KEY REFERENCES Customer(id),
          ...
          subtotal DECIMAL(10, 2), delivery DECIMAL(10, 2), total DECIMAL(10, 2),
          paymentStatus VARCHAR(50), status VARCHAR(50), date DATETIME
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='OrderItem' and xtype='U')
      CREATE TABLE OrderItem (
          id VARCHAR(50) PRIMARY KEY, orderId VARCHAR(100) FOREIGN KEY REFERENCES [Order](id),
          productId VARCHAR(50) FOREIGN KEY REFERENCES Product(id),
          productName VARCHAR(255), packSize VARCHAR(50), quantity INT, unitPrice DECIMAL(10, 2),
          totalPrice DECIMAL(10, 2), itemStatus VARCHAR(50)
      );

      IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='TrackingEvent' and xtype='U')
      CREATE TABLE TrackingEvent (
          id VARCHAR(50) PRIMARY KEY, orderId VARCHAR(100) FOREIGN KEY REFERENCES [Order](id),
          status VARCHAR(50), timestamp DATETIME, message VARCHAR(255)
      );
    `);
